...
- The library: shallow steps (2073914514 — homepage hero), spiral staircase (1629413118 — FAQ hero), ribbed curved sweep (2093142889 — homepage break + Platform case-study break), coffered grid ceiling (637731154 — About hero + FAQ pre-CTA break), concrete lattice (2063741601 — Work page hero), curved fins (1765844751 — Platform case-study hero). Every page hero uses a different image. Reuse across different pages is fine; max one photo per viewport. The founder may add more concrete/structural photos over time — same concept and treatment; drop sources in `marketing_images/`. FirstLight's case-study page is deliberately concrete-free — its future image is the founder's own nature/astro photography.
...
- **Section backdrops (founder rule, Aug 2026): use the imagery liberally as low-opacity texture BEHIND text sections** — flat two-tone black alternation is banned as a rhythm. The `Backdrop` component (0.08–0.22 opacity + dissolve scrims) replaces `bg-surface-alt` on textured sections. Rhythm: textured section → quiet section → textured. Animation sections (HowItWorks, JobWriter) stay quiet so motion owns the viewport.
- Max one STRONG image moment per viewport (hero/full-bleed); faint backdrops don't count against this. Currently dark-theme only (`.dark-only`) — light theme runs clean paper until a light grade is designed.
- Backdrop pool: curve (187297400), struts (845274473), shadow stairs (348118618) + reuse of the hero set at low opacity.
