...
export type DodItem = { key: string; label: string; ok: boolean; manual?: 'readme' | 'tests' }

export function dod(b: Brief): DodItem[] {
  return [
    { key: 'live', label: 'On production', ok: b.stage === 'Production' || b.stage === 'Complete' },
    { key: 'signoff', label: 'Signed off by the business SMEs', ok: b.signOff === 'signed off' },
    ...
    { key: 'smes', label: 'Business SMEs named', ok: b.smes.length > 0 },
    { key: 'trail', label: 'Audit trail from brief to production', ok: b.changeLog.length > 0 },
  ]
}
