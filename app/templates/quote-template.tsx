...
const NetworkCanvas: React.FC = () => {
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < numParticles; i++) {
      const isBright = Math.random() < 0.3 // Increased to 30% bright particles
      particles.push({
        x: Math.random() * width,
        y: Math.random() * height,
        vx: (Math.random() - 0.5) * 0.3,
        vy: (Math.random() - 0.5) * 0.3,
        radius: isBright ? Math.random() * 2 + 1.5 : Math.random() * 1.5 + 0.8,
        opacity: 1, // Full opacity for all particles
        isBright,
      })
    }
    ...
    const draw = () => {
      ...
      particles.forEach((p, idx) => {
        ...
        if (p.isBright) {
          ...
          // Brighter cyan/blue with full opacity
        ...
        // Connect to other particles - using brighter lines
